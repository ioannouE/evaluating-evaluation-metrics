...
parser.add_argument('--perturbation', type=float, default=0)
...
def get_activations(files, model, batch_size=1, dims=64,
                    cuda=False, verbose=False, noise=False, perturbation=0.):
    ...
    for i in range(n_batches): # tqdm(range(n_batches)):
        ...
        # visualize_image(batch.squeeze() * 255)
        if cuda:
            batch = batch.cuda()
            # batch = batch * 255
            if (noise):
                ...
                # # perturbed_image = (batch*255) + perturbation.cuda()
                # perturbed_image = (batch) + perturbation.cuda()
                # perturbed_image = torch.clamp(perturbed_image, 0, 1)
                # batch = perturbed_image

                # salt and pepper
                # batch = torch.tensor(random_noise((batch.cpu().numpy()*255), mode='s&p', amount=0.1*args.perturbation, clip=True))
                # batch = torch.tensor(random_noise((batch.cpu().numpy()), mode='s&p', amount=0.1*args.perturbation, clip=True))
                # batch = batch.cuda()
                
                # visualize_image(batch.squeeze())
                ...
                # batch = gaussian_blur((batch*255))

              
                batch_255 = batch # * 255
                # gaussian noise
                # stylized_image = torch.tensor(random_noise(batch_255.cpu().numpy(), mode='gaussian', mean=0, var=0.1*perturbation, clip=True))
                # batch = stylized_image.float().cuda()

                # implanted black rectangles
                batch = utils.add_black_rectangles_tensor(batch_255.clone().squeeze(), 10+int(10*perturbation), (10+int(30*perturbation), 10+int(30*perturbation)))
                batch = batch.unsqueeze(0)

                # swirl
                # image_np = batch_255.squeeze().permute(1, 2, 0).cpu().numpy()
                ...
                # batch = torch.tensor(swirled).permute(2,0,1)
                # batch = batch.unsqueeze(0).cuda()

                utils.visualize_image(batch.squeeze())
...
def calculate_activation_statistics(files, model, batch_size=1,
                                    dims=64, cuda=False, verbose=False, noise=False, perturbation=0.):
    ...
    """
    act = get_activations(files, model, batch_size, dims, cuda, verbose, noise=noise, perturbation=perturbation)
...
def calculate_sifid_given_paths(path1, path2, batch_size, cuda, dims, suffix, noise=False, perturbation=0.):
    ...
    # files2 = sorted([f for f in list(path2.glob(f'*.{suffix2}')) if 'composition_vii' in f.name])
    # print(files2[0], files1[0])


    fid_values = []
    Im_ind = []
    for i in tqdm(range(len(files2))):
        m1, s1 = calculate_activation_statistics([files1[i]], model, batch_size, dims, cuda, noise=False, perturbation=0.)
        m2, s2 = calculate_activation_statistics([files2[i]], model, batch_size, dims, cuda, noise=noise, perturbation=perturbation)
...
if __name__ == '__main__':
    ...
    sifid_values = calculate_sifid_given_paths(path1,path2,1,args.gpu!='',64,suffix, noise=args.noise, perturbation=args.perturbation)

    sifid_values = np.asarray(sifid_values,dtype=np.float32)
    numpy.save('SIFID', sifid_values)
    print('------------------------------------------------------------------------------------------------------------')
    print('Perturbation: ', args.perturbation, '   SIFID: ', "{:.4f}".format(sifid_values.mean()))
    print('------------------------------------------------------------------------------------------------------------')
