...
def get_activations(files, model, batch_size=1, dims=64,
    ...
    for i in range(n_batches): # tqdm(range(n_batches)):
        ...
        #images = images[0,:,:,:]
        # !!! Commented out to match the implementation in evaluate_sifid_average.py
        #images /= 255 # !!! Commented out to match the implementation in evaluate_sifid_average.py
...
def calculate_sifid_given_paths(path1, path2, batch_size, cuda, dims, suffix):
    ...
    # files2 = sorted([f for f in list(path2.glob(f'*.{suffix}')) if 'the_scream' in f.name])
