...
def main():
            ...
            for perturbation in perturbation_levels:
                if perturbation == 0:
                    ...
                    if is_random_noise(noise_type):
                        total_content_error = 0
                        for run in range(args.runs):
                            perturbed_image = apply_noise(original_image.squeeze(0), noise_type, perturbation).unsqueeze(0)
                            features_org = vgg(original_image)
                            features_perturbed = vgg(perturbed_image)
                            content_error = mse_loss(features_perturbed.relu2_2, features_org.relu2_2)
                            total_content_error += content_error.item()
                        ...
                        avg_content_error = total_content_error / args.runs
                    else:
                        perturbed_image = apply_noise(original_image.squeeze(0), noise_type, perturbation).unsqueeze(0)
                        features_org = vgg(original_image)
                        features_perturbed = vgg(perturbed_image)
                        avg_content_error = mse_loss(features_perturbed.relu2_2, features_org.relu2_2).item()
                    ...
                    results[img_path][noise_type][perturbation] = avg_content_error
                    print(f'Image: {os.path.basename(img_path)}, Noise: {noise_type}, Perturbation: {perturbation:.2f}, Avg Content Error: {avg_content_error:.4f}')

    # Export results to CSV
    # with open('content_error_results.csv', 'w', newline='') as csvfile:
    #     writer = csv.writer(csvfile)
    #     header = ['Image'] + [f'{noise}_{level}' for noise in noise_types for level in perturbation_levels]
    #     writer.writerow(header)

    #     for img_path, noise_results in results.items():
    #         row = [os.path.basename(img_path)]
    #         for noise_type in noise_types:
    #             for level in perturbation_levels:
    #                 row.append(f'{noise_results[noise_type][level]:.4f}')
    #         writer.writerow(row)
    ...
    with open('content_error_results.csv', 'w', newline='') as csvfile:
        ...
        for img_path in image_paths:
            ...
            for level in perturbation_levels:
                row = [level]
                for noise_type in noise_types:
                    error = results[img_path][noise_type][level]
    ...
    print("Results exported to content_error_results.csv")
