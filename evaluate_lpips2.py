...
def main():
    ...
    #############################################################################
    # benchmark_dir = '../../Unity/arbitrary-nst-test/content/' # 'testing_images/tested_image' # 'benchmark_unstylized'
    benchmark_dir = 'testing_images/fid_all'
    # retrieve the original frames
    original_frames_path = benchmark_dir + '/*.*'
    original_frames = []
    ...
    for stylized_img in sorted(glob.glob(test_frames_path + '/*.*')): 
        ...
        for content_img in original_frames:
            ...
            if desired_part in stylized_img:
                ...
                # added code here for content VS content 
                # gaussian_blur = transforms.GaussianBlur(kernel_size=(5+30,5+30), sigma=args.perturbation)
                # content_image_noisy = gaussian_blur(content_image)

                # gaussian noise
                # content_image_noisy = torch.tensor(random_noise(content_image, mode='gaussian', mean=0, var=0.1*args.perturbation, clip=True))
                # content_image_noisy = content_image_noisy.float()

                # utils.visualize_image(content_image_noisy)
                # sys.exit(0)
                
                # content_image_noisy = content_image_noisy.unsqueeze(0).to(device)
                # content_image_noisy = content_image_noisy * 2.0 - 1


                content_image = content_image.unsqueeze(0).to(device)
                
                # dist += ssim(content_image, stylized_image)
                content_image = content_image * 2.0 - 1
                stylized_image = stylized_image * 2.0 - 1
                
                lpips_error = lpips_sim.forward(content_image, stylized_image)
                dist += lpips_error # lpips_sim.forward(content_image, stylized_image)
                # dist += lpips_sim.forward(content_image, content_image_noisy)
                print('-------------------------')
                print('Image: ', desired_part, ' ---- LPIPS : {:.4f}'.format(lpips_error.item()))
                print('-------------------------')



    print('--------------------------------------------------------------')
    print('Average LPIPS: {:.4f}'.format(dist.item()/len(stylized_images)), ' ( Perturbation: {:.2f})'.format(args.perturbation))
    print('--------------------------------------------------------------')
