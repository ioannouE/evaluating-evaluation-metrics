...
class Inception3(nn.Module):
    def __init__(
        ...
        if aux_logits:
            self.AuxLogits1 = inception_aux(768, num_classes1)
            self.AuxLogits2 = inception_aux(768, num_classes2)
    ...
    def _forward(self, x: Tensor, return_features: bool=False) -> Tuple[Tensor, Optional[Tensor]]:
        ...
        if self.AuxLogits1 is not None and self.AuxLogits2 is not None:
            if self.training:
                aux1 = self.AuxLogits1(x)
                aux2 = self.AuxLogits2(x)
        # N x 768 x 17 x 17
        x = self.Mixed_7a(x)
        # N x 1280 x 8 x 8
        x = self.Mixed_7b(x)
    ...
    def forward(self, x: Tensor, return_features: bool=False) -> InceptionOutputs:
        ...
        if torch.jit.is_scripting():
            ...
            return self.eager_outputs(out1, out2, aux1, aux2)
