...
with open(csv_path, 'r') as file:
    ...
    for row in reader:
        ...
        style_label = row['style'].strip()
        
        # If the artist label is not in the label map, add it with a new integer key
        if style_label not in label_map and len(label_map) < num_classes:
            label_map[style_label] = len(label_map)
...
# with open("label_map_style.py", "w") as file:
#     file.write("label_map = " + str(label_map))

with open('labels_dicts/styles_109_dict.json', 'w', encoding='utf8') as handle:
