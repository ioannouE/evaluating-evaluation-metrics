        ...
        for noise_type in noise_types:
            for perturbation in perturbation_levels:
                if perturbation == 0:
                    ...
                    avg_ssim = 1.0  # SSIM is 1 for identical images
                else:
                    if is_random_noise(noise_type):
                        ...
                        for run in range(args.runs):
                            ...
                            ssim_value = ssim(original_image, perturbed_image)
                        ...
                        avg_ssim = total_ssim / args.runs
                    else:
                        perturbed_image = apply_noise(original_image.squeeze(0), noise_type, perturbation).unsqueeze(0)
                        avg_ssim = ssim(original_image, perturbed_image).item()
                    ...
                    results[img_path][noise_type][perturbation] = avg_ssim
                    print(f'Image: {os.path.basename(img_path)}, Noise: {noise_type}, Perturbation: {perturbation:.2f}, Avg SSIM: {avg_ssim:.4f}')

    # Export results to CSV
    with open('ssim_results.csv', 'w', newline='') as csvfile:
        ...
        for img_path in image_paths:
            ...
            for level in perturbation_levels:
                row = [level]
                for noise_type in noise_types:
                    ssim_value = results[img_path][noise_type][level]
                    row.append(f'{ssim_value:.4f}')
                writer.writerow(row)
            
            # Add an empty line between tables
            writer.writerow([])
    
    print("Results exported to ssim_results.csv")
